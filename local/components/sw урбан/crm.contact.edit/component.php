		...
		if(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']))
		{
			...
			if (empty($arResult['ERROR_MESSAGE']))
			{
				...
				else
				{
					...
					$DATA["OWNER_ID"]= $ID;
					$DATA["OWNER_TYPE"]= "CONTACT";
                    $DATA["OWNER_TYPE_ID"]= 3; 
                    $DATA["TYPE_ID"]= 2; 
                    $temp_exp=explode('.', $_REQUEST["vstavka"]);
                    $DATA["SUBJECT"]= $temp_exp[0];
                    $DATA["END_TIME"]= $nowStr;
                    $DATA["START_TIME"]= $nowStr;
                    $DATA["NOTIFY_TYPE"]= 0;
                    $DATA["COMPLETED"]= "Y";
                    $DATA["PRIORITY"]= 2; 
                    $DATA["RESPONSIBLE_ID"]= intval($USER->GetID());
                    $DATA["DESCRIPTION"]= $_REQUEST["vstavka"];
                    $DATA["DESCRIPTION_TYPE"]= 1; 
                    $DATA["DIRECTION"]= 1; 
                    $DATA["storageTypeID"]= 3; 
                    $DATA["BINDINGS"][] = array("OWNER_TYPE_ID" => 3,"OWNER_ID" => $ID);
					//$DATA["FILES"]= array("fileData"=>"/new-message-1.mp3"); 
					...
					$ID_ACT=CCrmActivity::Add($DATA);
                    $DATA["ID"]= $ID_ACT; 

					CCrmActivity::SaveCommunications($ID_ACT, $arComms);
				/*n)p*/
				}
