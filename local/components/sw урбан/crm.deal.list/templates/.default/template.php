...
<script type="text/javascript">
	...
	function deal_p(i){
	    ...
	    var popup = new BX.PopupWindow("my-popup", null, {
	        ...
	        titleBar: {content: BX.create("span", {html: '<b>Просмотр сделки</b>', 'props': {'className': 'access-title-bar'}})},
	        closeByEsc : true,
	    ...
	    BX.ajax.get('/local/components/sw/crm.phone_search/templates/.default/deal.php?show=Y&ajax=Y&deal_id='+i, function(data) {
...
<?
	...
	foreach($arResult['DEAL'] as $sKey =>  $arDeal)
	{
		...
		$arActions[] =  array(
			...
			'ONCLICK' => "deal_p(".CUtil::JSEscape($arDeal['ID']).")",
			'DEFAULT' => true
		);
		if ($arDeal['EDIT']):
			$arActions[] =  array(
		...
		$resultItem = array(
			...
			'columns' => array(
				'DEAL_SUMMARY' => CCrmViewHelper::RenderInfo($arDeal['PATH_TO_DEAL_SHOW'], isset($arDeal['TITLE']) ? $arDeal['TITLE'] : ('['.$arDeal['ID'].']'), $arDeal['DEAL_TYPE_NAME'], '',"deal_p(".CUtil::JSEscape($arDeal['ID'])."); return false;"),
				...
				'TITLE' => '<a target="_blank" href="'.$arDeal['PATH_TO_DEAL_SHOW'].'"
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			...
			{
				dialogTitle: "<?=GetMessageJS('CRM_DEAL_REBUILD_STATISTICS_DLG_TITLE')?>",
				dialogSummary: "<?=GetMessageJS('CRM_DEAL_REBUILD_STATISTICS_DLG_SUMMARY')?>"
			};
