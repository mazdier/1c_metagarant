...
foreach($arResult['PRODUCT_ROWS'] as &$row)
{
	...
	if ($arResult['INVOICE_MODE'])
	{
		...
		$row['PRICE'] = round(doubleval($row['PRICE']), 2);
		$row['QUANTITY'] = round(doubleval($row['QUANTITY']), 2);
		$row['CUSTOMIZED'] = ($row['CUSTOM_PRICE'] === 'Y') ? 'Y' : 'N';
		$row['TAX_RATE'] = round(doubleval($row['VAT_RATE']), 2) * 100;
		$row['TAX_INCLUDED'] = 'Y';
		...
		$row['DISCOUNT_SUM'] = round(doubleval($row['DISCOUNT_PRICE']), 2);
		$row['DISCOUNT_RATE'] = \Bitrix\Crm\Discount::calculateDiscountRate(($exclusivePrice + $row['DISCOUNT_SUM']), $exclusivePrice);
		$ary['MEASURE_CODE'] = isset($row['MEASURE_CODE']) ? intval($row['MEASURE_CODE']) : 0;
	...
	else
	{
		...
		$discoutSum = $discoutRate < 100
			? \Bitrix\Crm\Discount::calculateDiscountSum($exclusivePrice, $discoutRate)
			: doubleval($row['DISCOUNT_SUM']);
	...
	$row["DESIGN_INFO"] = $obBoProductRows->getRow($row["ID"]);
	if ($row["DESIGN_INFO"]["DESIGN_PRICE"] > 0)
	{
		$totalDesignSum += $row["DESIGN_INFO"]["DESIGN_PRICE"];
	}
...
$arResult['TOTAL_WITH_DESIGN'] = $totalDesignSum + $arResult['TOTAL_SUM'];
