...
<? if(!isset($_REQUEST["ajax"]) || $_REQUEST["ajax"] != "Y"): ?>
    ...
    { 
		var try_limit = getFilePathCounter++ > 10;
		
        if (isCreatedPhone || try_limit)
        {
			if(try_limit) console.log("Не удалось создать звонок, сервер Infinity не содержит активное соединение с заданными параметрами. ");
        ...
        $.getJSON(  '/infinity/ajax/ajax_create_agent.php',		//'<?=$templateFolder?>/ajax_create_agent.php',
		{
            url: '<?= $arParams["INFINITY_ADDRESS"]; ?>',
            IDUser: '<?= $arParams["INFINITY_USER_ID"]; ?>',
            ...
            { 
                ...
                }else console.log("can't get CrmActivityEditor");
