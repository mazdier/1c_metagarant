...
class CPhoneRecording
{
    ...
    const INFINITY_API_ADDRESS = "http://192.168.200.204:10080";
    const INFINITY_REC_USER = 'Администратор';
    const INFINITY_REC_PASS = 'InfinityX14';
    const INFINITY_REC_ADDR = "192.168.200.204";

    public $_infinityApiAddress = "http://192.168.0.182:10080";
    protected $_infinityRecAddress = "192.168.200.204";
    ...
    public static function AttachRecordToActivity($callRelationId, $AddressInfinity, $WebHook)
    {
        ...
        AddMessage2Log("starting agent with callRelationId = ".$callRelationId."\n");*/
        ...
        if (!self::_isActiveCall($arRelation["UF_INFINITY_USER_ID"], $arRelation["UF_INFINITY_CALL_ID"],$AddressInfinity))
        {
			...
			/*
            if (self::_updateActivity($arRelation["UF_ACTIVITY_ID"], $arRelation["UF_INFINITY_A_FILE"], $arRelation["UF_INFINITY_B_FILE"]))
            {
                return "";
            }*/
        }
        return "CPhoneRecording::AttachRecordToActivity(".$callRelationId.",".$AddressInfinity.");";
    }
    ...
    protected function _isActiveCall($userId, $callId,$AddressInfinity)
    {
		...
		$url = $AddressInfinity.'/call/getactivecalls/?IDUser='.$userId;
	...
	public function _fillCallOnFinished($callid, $activityId, $AddressInfinity, $WebHook)
	{
		...
		if (isset($arResponse["result"]["Data"]) && is_array($arResponse["result"]["Data"]) && count($arResponse["result"]["Data"]))
		{
			...
			//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/local/log_infinity.txt", "CCrmActivity $activityId : ".print_r($arActivity,true)."\n", FILE_APPEND);
			...
			if(is_array($provider_params))
			{
				...
				foreach($arResponse["result"]["Data"] as $arConnData)
				{
					...
					//$end_time = floatval($connect['TimeStartDate'])+floatval($connect['DurationTalk'])+floatval($connect['TimeStartTime']);
				...
				//$arFields["END_TIME"] = date("c", $end_time);
...
class CheckStatusCall {
	...
	$queryUrl = "http://192.168.200.204:10080/call/getactivecalls";
	$queryData = http_build_query($queryParams = array(
	...
	if($isCall)
	{
		...
		$queryData = http_build_query($queryParams1 = array(
			'url' => 'http://192.168.200.204:10080',
...
class CheckCallID {
    ...
    function Check($number) 
	{
		$ExtensionUser = 0;
		$phoneid = 0;
		$queryUrl = "http://192.168.200.204:10080/data/getdata/";
		$queryData = http_build_query($queryParams = array(
    ...
    function Update($ID_ACT) 
	{
          ...
          //  global $USER;
           ...
           $parent = $DB->query(sprintf(
                "UPDATE b_crm_act SET PROVIDER_ID = 'INFINITY_CALL' WHERE ID = ".$ID_ACT
            ));
