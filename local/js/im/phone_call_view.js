...
(function() {
	...
	BX.PhoneCallView.prototype.createLayout = function() {
		if (this.isFolded())
			...
			return this.createLayoutCrm() + this.ajax_zapros_facebook(); //add in layout
		else
	...
	BX.PhoneCallView.prototype.createLayoutCrm = function() {
				...
				BX.create("div", {
					...
					children: [
						BX.create("div", {
							...
							children: [
								BX.create("div", {
									props: { className: 'im-phone-call-header-container' },
									children: [
										BX.create("div", {
											props: { className: 'im-phone-call-header' },
											children: [
												...
												BX.create("div", {
													props: { className: 'facebook__window__bg' },
													children: [
														BX.create("div", {
															props: { className: 'facebook__ajax1' },
															style: {
																margin: '0',
																display: 'block',
																color: '#fff',
																fontFamely: 'sans-serif',
																fontSize: '9pt',
																background: '#232931',
																maxHeight: '70px',
																overflow: 'auto'
															},
															html: 'Facebook-wait-server...'
														})
													]
												}),
												BX.create("div", {
													props: { className: 'combox_place' },
													children: [
														BX.create("div", {
															props: { className: 'combox_place__ajax1' },
															style: {
																margin: '0',
																display: 'block',
																color: '#fff',
								...
								this.elements.sections.status = BX.create("div", {
									props: { className: 'im-phone-call-section' },
									style: this.sections.status.visible ? {} : { display: 'none' },
									children: [
										BX.create("div", {
											props: { className: 'im-phone-call-status-description' },
											children: [
								...
								this.elements.sections.timer = BX.create("div", {
									props: { className: 'im-phone-call-section' },
									style: this.sections.timer.visible ? {} : { display: 'none' },
									children: [
										BX.create("div", {
											props: { className: 'im-phone-call-status-timer' },
											children: [
												BX.create("div", {
													props: { className: 'im-phone-call-status-timer-item' },
													children: [
								...
								//this
								this.elements.sections.addNewButtons = BX.create("div", {
									props: { className: 'im-phone-add-buttons' },
									children: [
										this.elements.sections.addNewButtons = BX.create("div", {
											props: { className: 'add-buttons-bg' },
											children: [
												this.elements.sections.addNewButtons = BX.create("div", {
													props: { className: 'pausePhone displayLine' },
													children: [
														this.elements.sections.addNewButtons = BX.create("div", {
															props: { className: 'pausePhone__style' },
															html: 'Pause',
															children: [
																this.elements.sections.addNewButtons = BX.create("i", {
																	props: { className: 'ion-ios-pause iconSize' },
												...
												this.elements.sections.addNewButtons = BX.create("div", {
													props: { className: 'exitPhone displayLine' },
													children: [
														this.elements.sections.addNewButtons = BX.create("div", {
															props: { className: 'exitPhone__style' },
															html: 'Exit',
															children: [
																this.elements.sections.addNewButtons = BX.create("i", {
																	props: { className: 'ion-md-close iconSize weightBeforeFix' },
								...
								this.elements.sections.crmButtons = BX.create("div", {
									props: { className: 'im-phone-call-section' },
		...
		setTimeout(function() { startInfinityCustomFunctions(); }, 0); //START_CUSTOM
		return result;
	};


	/* ====================================== *
	 *	 CUSTOM OF THE PHONE CALL WINDOW
	 *  */

	function startInfinityCustomFunctions() {
		setReworkStyleCSS();
		loadTitlePhoneNumber();
	}

	function setReworkStyleCSS() {
		customStyle.LinkIconFonts();
		customStyle.FixCrmCardPadding();
		customStyle.buttonWidth();
		customStyle.addFontsIcons.bgColorOrb();
		customStyle.addFontsIcons.bgColorOrbHover();
		customStyle.removePadding.callHeader();
		customStyle.ajaxeBlockImg.replaceImgSize();
	}

	var customStyle = {
		removePadding: {
			callHeader: function() {
				$('.im-phone-call-header').css('padding', '0');
			}
		},
		ajaxeBlockImg: {
			replaceImgSize: function() {
				$('.facebook__ajax1 img').css({
					width: '70px',
					height: 'auto'
				});
				$('.facebook__ajax1 table').css({
					fontSize: '0',
					display: 'inline-block',
					margin: '0',
					padding: '0',
				});
				$('.facebook__ajax1 tbody tr td')
		...
		LinkIconFonts: function() {
			$('head').html('  <link rel="stylesheet" type="text/css" href="https://infinitered.github.io/ionicons-version-3-search/css/ionicons.min.css"> ');
		},
		FixCrmCardPadding: function() {
			$('.crm-card-show-detail-info').css('padding', '0');
			$('input#u_0_0').hide();
		},
		colorSelector: function() {
			$('div.combox_place__ajax1 > form').css('background', '#29303a');
			$("select#DEP").css({
				border: 'transparent',
				width: '100%',
				display: 'inline-block',
		...
		buttonWidth: function() {
			...
			$('.weightBeforeFix::before').css('fontWeight', 'bold');
		},
		addFontsIcons: {
			bgColorOrb: function() {
				$('.pausePhone__style, .exitPhone__style').css({
					width: '40px',
					height: '40px',
					lineHeight: '40px',
					background: '#232931',
			...
			bgColorOrbHover: function() {
				$('.pausePhone__style, .exitPhone__style')
					.hover(function() { $(this).css('background', '#313a46'); },
						function() {
							$(this).css('background', '#232931');
						});
			}
		}
	};
	...
	var setPhoneClear = '';
	var getPhoneTitle = document.getElementsByClassName('im-phone-call-title-text');

	function loadTitlePhoneNumber() {
		var myurl = document.getElementById('dummy_url');
		var facebookURL = 'https://www.facebook.com/search/top/?q=';

		for (var i = 0; i < getPhoneTitle.length; i++) {
			setPhoneClear = getPhoneTitle[i].innerText.match(/\d+$/)[0];
			console.log("text: " + setPhoneClear);
		}
		...
		function openLinksInNewTabs() {
			...
			myurl.target = '_blank';
			myurl.innerHTML = "facebook";
			myurl.setAttribute('href', facebookURL + setPhoneClear);
			return false;
		}
		openLinksInNewTabs();

		var dd = $.Callbacks();
		dd.add(ajax_zapros_facebook, [ajax_zapros_comboBox]);
		dd.fire(ajax_zapros_facebook, [ajax_zapros_comboBox]);


	}

	function ajax_zapros_facebook() {
		var b = $('.facebook__ajax1');
		var id_phone=$('#id_lead_phone:hidden').text();
		console.log(id_phone);
		$.ajax({
			type: 'POST',
			url: 'https://crm.metegerant.by/bitrix/facebookapp/curl.php',
			data: { 'PHONE': setPhoneClear, },//'PHONE_ID': id_phone
			success: function(data) {
				b.html(data);
				var image_src=$('._5s61._1nhd._5jps._52we').find('img').eq(0).attr("src");
				var full_name=$('._4g34._1nhc._52we').find('a').eq('0').text();
				console.log(full_name);
				$.ajax({
					type: 'POST',
					url: '/local/js/im/ajax_phone_lead.php',
					data: { 'FullName': full_name, 'IDPhone':id_phone, 'Image': image_src },
					success: function(data) {
						
					}
				});
				if (b.html() == false) {
					b.html('Пользователь не найден!');
				}
	...
	function ajax_zapros_comboBox() {
		var a = $('.combox_place__ajax1');
		//var isUrl = 'https://nfr.servit.by/local/components/sw/infinity.block/templates/.default/ajax_department.php';
		var isUrl = '/infinity/ajax/ajax_department.php';
		var isUrl2 = '/infinity/ajax/ajax_redirect_call.php';
		...
		$.ajax({
			...
			success: function(data) {
				a.html(data);
				if (a.html() == false) {
					a.html('Пользователь не найден!');
				}
				setTimeout(function() {
					var ff = $('#DEP option:selected');
					var aa = ff.val();

					//colors
					customStyle.colorSelector();

					//event.select.user
					$("select#DEP").change(function() {
						console.log(ff.val());
						console.log(ff.text());
						console.log(PCW.callId);
						console.log(PCW.crmActivityId);
						console.log(PCW.phoneNumber);
						...
						$.ajax({
							...
							url: isUrl2,
							data: {
								'PHONE_NUMBER': PCW.phoneNumber,
								'CALL_ID': PCW.callId,
								'CRM_ACTIVITY_ID': PCW.crmActivityId,
								'Extension': $('#DEP option:selected').val()
							},
							success: function(data) {
	...
	/*  *
	 *	 CUSTOM OF THE PHONE CALL WINDOW 	//END
	 * ====================================== */
	...
	BX.PhoneCallView.prototype.createSidebarLayout = function() {
		...
		this.elements.sidebarContainer = BX.create("div", {
			props: { className: 'im-phone-sidebar-wrap' },
			children: [
				BX.create("div", {
					props: { className: 'im-phone-sidebar-tabs-container' },
					children: [
						this.elements.tabsContainer = BX.create("div", { props: { className: 'im-phone-sidebar-tabs-left' }, children: tabs }),
						BX.create("div", {
							props: { className: 'im-phone-sidebar-tabs-right' },
							children: [
	...
	BX.PhoneCallView.prototype.createLayoutSimple = function() {
		...
		var result = BX.create("div", {
			props: { className: 'im-phone-call-wrapper' },
			children: [
				BX.create("div", {
					props: { className: 'im-phone-call-container' },
					children: [
						BX.create("div", {
							props: { className: 'im-phone-calling-section' },
							children: [
								this.elements.title = BX.create("div", { props: { className: 'im-phone-calling-text' } })
							]
						}),
						BX.create("div", {
							props: { className: 'im-phone-call-section im-phone-calling-progress-section' },
							children: [
								BX.create("div", {
									props: { className: 'im-phone-calling-progress-container' },
									children: [
										BX.create("div", {
	...
	BX.PhoneCallView.prototype.createLayoutFolded = function() {
		...
		return BX.create("div", {
			props: { className: "im-phone-call-panel-mini-container" },
			children: [
				BX.create("div", {
					props: { className: "im-phone-call-panel-mini-button-main" },
					children: [
						BX.create("div", {
	...
	BX.PhoneCallView.prototype.createTitle = function() {
		...
		if (this.phoneNumber == 'hidden') {
			...
			if (callTitle.substr(0, 1) == '8' || callTitle.substr(0, 1) == '+') {} else if (!isNaN(parseInt(callTitle)) && callTitle.length >= 10) {
				callTitle = '+' + callTitle;
			}
		}
	...
	BX.PhoneCallView.prototype.renderProgress = function() {
		...
		if (progress == 'connect') {
			result = BX.create("div", {
				props: { className: 'bx-messenger-call-overlay-progress' },
				children: [
			...
			switch (buttonName) {
				...
				case 'qualityMeter':
					buttonNode = BX.create("span", {
						...
						children: [
							BX.create("span", {
								props: { className: 'im-phone-call-btn-signal-icon-container' },
								children: [
				...
				case 'fold':
					if (!self.isDesktop() && self.canBeFolded()) {
						buttonNode = BX.create("div", {
							props: { className: 'im-phone-btn-arrow active' },
							children: [
	...
	Desktop.prototype.getHtmlPage = function(content, jsContent, initImJs, bodyClass) {
		...
		var initJQ = '';
		if (initImJs == true) {
			initJQ = "<script src=https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js></script><div id='id_lead_phone' style='display:none;'>"+this.parentPhoneCallView.crmEntityId+"</div>";
		...
		return '<!DOCTYPE html><html>' + this.htmlWrapperHead + '<body class="im-desktop im-desktop-popup ' + bodyClass + '"><div id="placeholder-messanger">' + content + '</div>' + initJs + jsContent + initJQ + '</body></html>';
	};
	...
	CallList.prototype.renderCallListItems = function(statusId) {
		...
		if (status.ITEMS.length > status._shownCount) {
			status._showMoreNode = BX.create("div", {
				props: { className: 'im-phone-call-list-show-more-wrap' },
				children: [
					BX.create("span", {
		...
		itemDescriptor._node = BX.create("div", {
			...
			children: [
				BX.create("div", {
					...
					children: [BX.create("span", { text: statusName })],
					events: {
						click: function(e) {
	...
	BX.FoldedCallView.prototype.render = function(animation) {
		...
		this.node.appendChild(BX.create("div", {
			...
			children: [
				BX.create("div", {
					props: { className: 'im-phone-call-wrapper-fixed-user' },
					children: [
						BX.create("div", {
							props: { className: 'im-phone-call-wrapper-fixed-user-image' },
							children: [
		...
		this.node.appendChild(BX.create("div", {
			props: { className: 'im-phone-call-wrapper-fixed-right' },
			children: [
				BX.create("div", {
					props: { className: 'im-phone-call-wrapper-fixed-btn-container' },
					children: [
		...
		this.node.appendChild(BX.create("div", {
			props: { className: 'im-phone-btn-block' },
			children: [
	...
	Keypad.prototype.render = function() {
		...
		return BX.create("div", {
			...
			children: [
				BX.create("div", {
					props: { className: "bx-messenger-calc-body" },
					children: [
						this.elements.inputContainer = BX.create("div", {
							props: { className: 'bx-messenger-calc-panel' },
							children: [
								BX.create("span", {
									props: { className: "bx-messenger-calc-panel-delete" },
									events: {
				...
				this.hideDial ? null : BX.create("div", {
					props: { className: "bx-messenger-call-btn-wrap" },
					children: [
						BX.create("span", {
